...
namespace usub::unet::ws {
    ...
    ClientFrameParser::ClientFrameParser(std::size_t maxPayload)
        : max_payload_(maxPayload) {}

    ClientFrameParser::ParserContext &ClientFrameParser::getContext() noexcept {
        return ctx_;
    }

    const ClientFrameParser::ParserContext &ClientFrameParser::getContext() const noexcept {
        return ctx_;
    }

    void ClientFrameParser::reset() noexcept {
        ctx_ = {};
    }

    std::expected<bool, FrameParseError> ClientFrameParser::step(
            ClientFrame &frame,
        ...
        while (begin != end) {
            ...
            case STATE::BYTE1: {
                ...
                if (!masked) {
                    ctx_.state = STATE::FAILED;
                    return std::unexpected(FrameParseError{
                            .code = FrameParseError::CODE::UNMASKED_CLIENT_FRAME,
                            .message = "client frame must be masked (RFC 6455 section 5.1)"});
                }
            ...
            case STATE::MASK_KEY:
                frame.mask_key[ctx_.mask_idx++] = c;
                if (ctx_.mask_idx == 4) {
            ...
            case STATE::PAYLOAD: {
                const auto maskPos = static_cast<uint8_t>(frame.payload.size() % 4u);
                frame.payload.push_back(static_cast<char>(c ^ frame.mask_key[maskPos]));
