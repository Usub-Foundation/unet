...
// Enforces that every frame carries a mask (RFC 6455 §5.1).
...
namespace usub::unet::ws {
    ...
    struct FrameParseError {
        enum class CODE : uint8_t {
            ...
            PAYLOAD_TOO_LARGE,
            UNMASKED_CLIENT_FRAME,  // server received unmasked frame — RFC 6455 §5.1
            MASKED_SERVER_FRAME,    // client received masked frame  — RFC 6455 §5.1
            FAILED_STATE,
        };
        CODE        code{};
        std::string message{};
    ...
    public:
        ...
        explicit ClientFrameParser(std::size_t max_payload = DEFAULT_MAX_PAYLOAD)
        ...
        step(ClientFrame &frame,
            ...
            while (begin != end) {
                ...
                case STATE::BYTE1: {
                    ...
                    if (!masked) {
                        ...
                        return std::unexpected(FrameParseError{
                            ...
                            .message = "client frame must be masked (RFC 6455 §5.1)"});
                    }
                ...
                case STATE::PAYLOAD: {
                    const auto mask_pos = static_cast<uint8_t>(frame.payload.size() % 4u);
                    frame.payload.push_back(static_cast<char>(c ^ frame.mask_key[mask_pos]));
