...
namespace usub::unet::http {
    ...
    public:
        ...
        request(const std::string &host, std::uint16_t port, Request request,
            ...
            co_return co_await this->requestWithStream(stream_instance, host, port, std::move(request), options);
        }
            ...
            requires(sizeof...(Streams) > 0)
        {
            ...
            co_return co_await this->request<DefaultStream>(host, port, std::move(request), options);
        }
    ...
    private:
        ...
        requestWithStream(Stream &stream_instance, const std::string &host, std::uint16_t port, Request request,
            ...
            if (request.metadata.uri.path.empty() && request.metadata.authority.empty()) { request.metadata.uri.path = "/"; }
            ...
            while (parser.getContext().state != v1::ResponseParser::STATE::COMPLETE) {
                ...
                if (read_size == 0) {
                    auto finalize = parser.finalize(response);
                    if (!finalize) {
                        if (options.close_after_response) { co_await stream_instance.shutdown(socket); }
                        co_return std::unexpected(ClientError{
                                .code = ClientError::CODE::PARSE_FAILED,
                                .message = finalize.error().message,
                                .parse_error = finalize.error(),
                        });
                    }
                    break;
                }
